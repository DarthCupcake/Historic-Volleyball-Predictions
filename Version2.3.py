...
def URL(url):
    ...
    theURL = pd.read_html(theURL.text)
    # htmlParser = BeautifulSoup(theURL.text,'html.parser')
    # Sets(htmlParser, finaldf)
    Sets(theURL, finaldf)

def Sets(htmlParser, dataframe):
    Opponent1 = 0
    Opponent2 = 0
    whoWonTheSet = htmlParser[0]
    # whoWonTheSet = whoWonTheSet.iat[1,1]
    numOfGames = len(whoWonTheSet.axes[1]) - 1
    # f = open("file.txt", "w")
    # f.write(str(htmlParser[0]))
    # f.close()
    # print(htmlParser)
    # print(htmlParser[-1].loc[:,["Score", "Serve Team"]])
...
# listOfYears = ["https://pepperdinewaves.com/sports/womens-volleyball/schedule/2020-21"]
...
# for i in gameLinks:
URL("https://pepperdinewaves.com/sports/womens-volleyball/stats/2020-21/san-francisco/boxscore/9838")


for i in finaldfarr:
    finaldf = finaldf.append(i, ignore_index=True)
